...
export default async function HomePage() {
  ...
  return (
    <AppShell
      currentPath="/"
      title="수업진도 체크"
      description="관리창에서 수업내용을 정리하고, 실행창에서 날짜 기준 누적 실행표를 만들며 1반부터 9반까지 진도를 빠르게 체크합니다."
    >
        ...
        <section className="panel p-6">
          ...
          <div className="mt-6 rounded-3xl border border-[var(--line)] bg-[linear-gradient(135deg,rgba(47,108,79,0.1),rgba(255,255,255,0.8))] p-5">
            ...
            <ol className="mt-4 space-y-3 text-sm leading-6 text-[var(--muted)]">
              <li>1. 관리창에서 수업내용 목록을 등록하고 정리합니다.</li>
              <li>2. 실행창에서 날짜, 수업내용, 메모를 정해 수업행을 추가합니다.</li>
              <li>3. 추가된 행은 날짜를 바꿔도 계속 누적된 상태로 유지됩니다.</li>
              <li>4. 각 반의 칸을 눌러 O, X, 빈칸 상태를 바로 기록합니다.</li>
              <li>5. 최근 진도 화면에서 반별 최신 진행 상황을 확인합니다.</li>
            </ol>
          </div>
        </section>
            ...
            {[
              {
                href: "/board",
                title: "실행창 열기",
                description: "날짜와 수업내용 기준으로 수업행을 추가하고 누적 실행표에서 반별 O/X를 기록합니다.",
              },
              {
                href: "/admin",
                title: "관리창 열기",
                description: "수업내용 목록을 등록하고 순서를 정리합니다.",
              },
              {
                href: "/progress",
                title: "최근 진도 보기",
                description: "1반부터 9반까지 마지막 기록 상태를 한 번에 확인합니다.",
              },
            ].map((item) => (
              <Link
                key={item.href}
